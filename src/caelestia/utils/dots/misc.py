...
def run_hooks(manifest: Manifest, kind: str, components: list[str] | None = None) -> None:
    """Run the global + enabled components' hooks of the given kind (e.g. "post_install").

    `components` restricts the run to a subset of the enabled components, used to
    give components enabled for the first time their `post_install` rather than
    the `post_update` everything else gets.
    """

    hooks = manifest.enabled_hooks(kind, components)
    if not hooks:
        return
